...
public abstract class ConfigFile {
	...
	// TODO: That is a derped approach. Use TreeSet with CASE_INSENSITIVE_ORDER
	// instead.
					...
					if (thisLine.toLowerCase().contains(":")) {
						...
						if (splitSettings.length == 2) {
							this.SettingsCache.put(splitSettings[0].trim()
									.toLowerCase(), splitSettings[1].trim());
							this.SettingsCache.put(splitSettings[0].trim(),
									splitSettings[1].trim());
	...
	protected List<WeightedMobSpawnGroup> ReadModSettings(String settingsName,
			List<WeightedMobSpawnGroup> defaultValue) {
		String json = this.SettingsCache.get(settingsName);
		if (json == null)
			return defaultValue;
		return gson.fromJson(json,
				new TypeToken<List<WeightedMobSpawnGroup>>() {
				}.getType());
	}

	protected ArrayList<String> ReadModSettings(String settingsName,
			ArrayList<String> defaultValue) {
		if (this.SettingsCache.containsKey(settingsName)) {
			ArrayList<String> out = new ArrayList<String>();
			if (this.SettingsCache.get(settingsName).trim().equals("")
					|| this.SettingsCache.get(settingsName).equals("None")) {
				return out;
			}
			Collections.addAll(out,
					this.SettingsCache.get(settingsName).split(","));
			return out;
		}
		return defaultValue;
	}
	...
	protected int ReadModSettings(String settingsName, int defaultValue) {
		...
		if (this.SettingsCache.containsKey(settingsName)) {
			try {
				...
				System.out.println("TerrainControl: " + settingsName
						+ " had wrong value");
			}
		}
		return defaultValue;
	...
	protected void WriteSettingsFile(File settingsFile) {
		try {
			this.SettingsWriter = new BufferedWriter(new FileWriter(
					settingsFile, false));
	...
	protected void WriteValue(String settingsName,
			ArrayList<String> settingsValue) throws IOException {
		String out = "";
	...
	protected void WriteValue(String settingsName,
			List<WeightedMobSpawnGroup> settingsValue) throws IOException {
		this.SettingsWriter.write(settingsName + ":"
				+ gson.toJson(settingsValue));
		this.SettingsWriter.newLine();
	}

	protected void WriteValue(String settingsName, int settingsValue)
			throws IOException {
		this.SettingsWriter.write(settingsName + ":"
	...
	protected void WriteValue(String settingsName, double settingsValue)
			throws IOException {
		this.SettingsWriter.write(settingsName + ":"
	...
	protected void WriteValue(String settingsName, float settingsValue)
			throws IOException {
		this.SettingsWriter.write(settingsName + ":"
				+ Float.toString(settingsValue));
		this.SettingsWriter.newLine();
	}

	protected void WriteValue(String settingsName, boolean settingsValue)
			throws IOException {
		this.SettingsWriter.write(settingsName + ":"
				+ Boolean.toString(settingsValue));
		this.SettingsWriter.newLine();
	}

	protected void WriteValue(String settingsName, String settingsValue)
			throws IOException {
	...
	protected void WriteColorValue(String settingsName, int RGB)
			throws IOException {
		this.SettingsWriter.write(settingsName
				+ ":0x"
				+ Integer.toHexString((0xFFFFFF & RGB) | 0x1000000)
						.substring(1));
		this.SettingsWriter.newLine();
	}
	...
	protected ArrayList<String> CheckValue(ArrayList<String> biomes,
			ArrayList<String> customBiomes) {
	...
	protected static void WriteStringToStream(DataOutputStream stream,
	...
	protected static String ReadStringFromStream(DataInputStream stream)
			throws IOException {
